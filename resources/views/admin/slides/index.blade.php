...
@section('main-content')
    <div class="box">
        <div class="content-header border-bottom pb-5">
            ...
            @can('SlideController@store')
                <a href="{{ url('/admin/slides/create') }}" class="btn btn-default float-right"
        ...
        <div class="box-header">
            <div class="box-tools">
                {!! Form::open(['method' => 'GET', 'url' => '/admin/slides', 'class' => 'pull-left', 'role' => 'search']) !!}
        ...
        @php($index = ($slides->currentPage() - 1) * $slides->perPage())
                ...
                <tbody>
                    ...
                    @foreach ($slides as $item)
                        <tr>
                            ...
                            @can('SlideController@show')
                                <td class="" style="width: 200px;"><a class="d-inline-block text-center"
                                        href="{{ url('/admin/slides') . '/' . $item->id }}" style="color: black;">
                                        <img class="w-100 object-fit-cover" src="{{ asset('storage/' . $item->pathimage) }}"
                                            alt="Image" />
                                    </a></td>
                            @endcan
                            ...
                            <td class="dropdown">
                                ...
                                <div class="dropdown-menu p-0">
                                    @can('SlideController@show')
                                        <a href="{{ url('/admin/slides/' . $item->id) }}"
                                    ...
                                    @can('SlideController@update')
                                        <a href="{{ url('/admin/slides/' . $item->id . '/edit') }}"
    ...
    <script>
        $(document).ready(function() {
            $('.btn-update-arrange').click(function() {
                ...
                $.ajax({
                    url: '{{ route('slides.updateArrange') }}',
                    method: 'POST',
                    data: {
                        id: id,
                        arrange: arrange,
